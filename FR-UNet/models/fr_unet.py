...
class FR_UNet(nn.Module):
    def __init__(self,  num_classes=1, num_channels=1, feature_scale=2,  dropout=0.2, fuse=True, out_ave=True):
        ...
        self.final5 = nn.Conv2d(
            filters[0], num_classes, kernel_size=1, padding=0, bias=True)
        self.fuse = nn.Conv2d(
            5, num_classes, kernel_size=1, padding=0, bias=True) # おそらく最終層の出力を平均化するための層
    ...
    def forward(self, x):
        ...
        if self.out_ave == True:
            ...
            output = self.fuse(torch.cat([self.final1(x1_1), self.final2(x10), self.final3(x11), self.final4(x12), self.final5(x13)], dim=1))
            # output = self.final5(x13)

        return output

def dual_threshold_iteration(prob_map, high_thresh=0.7, low_thresh=0.3, max_iter=10):
    """
    二重しきい値反復アルゴリズムの実装

    Args:
        prob_map (torch.Tensor): モデルの予測確率マップ (B, H, W)。
        high_thresh (float): 高いしきい値 (デフォルトは0.7)。
        low_thresh (float): 低いしきい値 (デフォルトは0.3)。
        max_iter (int): 最大反復回数 (デフォルトは10)。

    Returns:
        torch.Tensor: 最終的なセグメンテーションマップ (B, H, W)。
    """
    # 高いしきい値を適用して初期の強いセグメンテーションマップを作成
    strong_map = (prob_map >= high_thresh).float()

    # 低いしきい値の範囲内にあるピクセルを取得
    weak_map = ((prob_map >= low_thresh) & (prob_map < high_thresh)).float()

    # セグメンテーションマップを反復処理して更新
    for _ in range(max_iter):
        # 近傍のピクセルをチェックするために畳み込みを使用
        # カーネルは3x3で周囲8つのピクセルをチェック
        neighbor_kernel = torch.ones(1, 1, 3, 3, device=prob_map.device)
        neighbor_count = F.conv2d(strong_map.unsqueeze(1), neighbor_kernel, padding=1).squeeze(1)
        
        # 近傍に1がある弱いピクセルを1に更新
        new_strong_map = ((neighbor_count > 0) & (weak_map == 1)).float()

        # 新しい強いセグメンテーションマップを反映
        if new_strong_map.sum() == 0:  # もし変更がなければ終了
            break

        strong_map = strong_map + new_strong_map
        weak_map = weak_map - new_strong_map  # 更新されたピクセルは弱いマップから除外

    # 最終的に残ったピクセルを0に設定
    final_segmentation = (strong_map > 0).float()
