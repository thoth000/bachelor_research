...
class FR_UNet(nn.Module):
    def __init__(self,  args):
        ...
        self.vec1 = nn.Conv2d(
            filters[0], num_classes*2, kernel_size=3, padding=1, bias=True)
        self.vec2 = nn.Conv2d(
            filters[0], num_classes*2, kernel_size=3, padding=1, bias=True)
        self.vec3 = nn.Conv2d(
            filters[0], num_classes*2, kernel_size=3, padding=1, bias=True)
...
class Anisotropic_Diffusion(nn.Module):
    ...
    def forward(self, x):
        output, v_long = self.model(x)
        # v (B, 2, H, W)を正規化
        v_long = F.normalize(v_long + 1e-6, p=2, dim=1) # (B, 2, H, W)
        x = torch.sigmoid(output)
        # Allen-Cahn equation
        D = self.diffusion_matrix(1, 1, v_long)
        for _ in range(self.steps):
            # Runge-Kutta method 4th order
            k1 = self.allen_cahn(x, v_long, D)
            k2 = self.allen_cahn(x + self.dt/2 * k1, v_long, D)
            k3 = self.allen_cahn(x + self.dt/2 * k2, v_long, D)
            k4 = self.allen_cahn(x + self.dt * k3, v_long, D)
            x = x + self.dt/6 * (k1 + 2*k2 + 2*k3 + k4)
            
        # 相分離モデル化のために[0, 1]にクリッピング
        x = torch.clamp(x, 0, 1)
        return x, v_long, torch.sigmoid(output)