...
def evaluate(model, dataloader, criterion, epoch, args, device):
    ...
    with torch.no_grad():
        for i, sample in enumerate(dataloader):
            ...
            loss_grad = GradLoss()(preds, masks)

            loss = args.lambda_main * loss_main + args.lambda_grad * loss_grad
            ...
            if args.rank == 0 and (args.save_mask or epoch == args.max_epoch - 1):
                ...
                save_main_out_image(output, os.path.join(out_dir, f"{i}_origin.png"))
                save_main_out_image(output > args.threshold, os.path.join(out_dir, f"{i}_origin_binary.png"), cmap="gray")
                save_main_out_image(v_long[:, 0:1], os.path.join(out_dir, f"{i}_vec_x.png"))
                save_main_out_image(v_long[:, 1:2], os.path.join(out_dir, f"{i}_vec_y.png"))
