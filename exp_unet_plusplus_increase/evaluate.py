    ...
    with torch.no_grad():
        for i, sample in enumerate(dataloader):
            ...
            if args.rank == 0 and args.save_mask and i == 0:
                ...
                # save_binary_image(masks_gt, os.path.join(out_dir, f"masks_gt_{i}.png"))
...
def save_masked_output(output_tensor, mask_tensor, filepath, cmap="viridis"):
    """
    ...
    # カラーマップを使って画像として保存
    plt.imshow(masked_output, cmap=cmap, vmin=-1, vmax=1)  # vmin/vmaxで値域を明示
    plt.colorbar()
...
def save_masked_output_with_imsave(output_tensor, mask_tensor, filepath, normalize=True, cmap="viridis"):
    """
    ...
    # `plt.imsave` を使用して保存
    plt.imsave(filepath, masked_output, cmap=cmap_instance, vmin=-1, vmax=1)
