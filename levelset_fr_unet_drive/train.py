...
def train(args, writer=None):
    ...
    if args.pretrained_path is not None: # 事前学習済みモデルがある場合、optimizerで最終層のみを更新
        ...
        pretrained_state_dict = torch.load(args.pretrained_path)['state_dict']
    ...
    for epoch in progress_bar:
        ...
        if epoch % args.val_interval == args.val_interval - 1:
            val_loss, acc, sen, spe, iou, miou, dice = evaluate(model, valloader, epoch, args, device)
            if args.rank == 0:
                ...
                if val_loss < best_info['val_loss'] and epoch > args.pre_epoch:
