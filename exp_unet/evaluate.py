...
def dti(preds, thresh_low=0.3, thresh_high=0.5, max_iter=1000):
    ...
    for i in range(max_iter):
        new_fixed_mask = F.max_pool2d(fixed_mask, 3, 1, 1) * checkable_mask
        if torch.all(new_fixed_mask == fixed_mask):
            break
        else:
            fixed_mask = new_fixed_mask
    
    return new_fixed_mask
...
def evaluate(model, dataloader, criterion, epoch, args, device):
    ...
    with torch.no_grad():
        for i, sample in enumerate(dataloader):
            ...
            # loss用ではなく，評価指標用
            loss = torch.nn.BCELoss()(preds, masks_gt)
