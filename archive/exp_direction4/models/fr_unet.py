...
class FR_UNet(nn.Module):
    def __init__(self,  args):
        ...
        self.final_4 = nn.Conv2d(
            filters[0], num_classes, kernel_size=1, padding=0, bias=True)
        self.final_5 = nn.Conv2d(
            filters[0], num_classes, kernel_size=1, padding=0, bias=True)
        
        # 主方向拡散係数層
        self.s_long1 = nn.Conv2d(
            filters[0], num_classes, kernel_size=7, padding=3, bias=True)
        ...
        self.vec1 = nn.Conv2d(
            filters[0], num_classes*2, kernel_size=7, padding=3, bias=True)
        self.vec2 = nn.Conv2d(
            filters[0], num_classes*2, kernel_size=7, padding=3, bias=True)
        self.vec3 = nn.Conv2d(
            filters[0], num_classes*2, kernel_size=7, padding=3, bias=True)
    ...
    def forward(self, x):
        ...
        output = (self.final_1(x1_1)+self.final_2(x10)+self.final_3(x11)+self.final_4(x12)+self.final_5(x13)) / 5
        # 長手方向拡散係数層
...
class Anisotropic_Diffusion(nn.Module):
    ...
    def allen_cahn(self, u, s_long, v_long, energy):
        # Allen-Cahn方程式の計算
        # u (torch.Tensor): (B, C, H, W)
        # du = M * {div(gamma * grad(u)) - u(1-u)(2u-1)}
        dx, dy = self.gradient(u)
        D = self.diffusion_matrix(s_long, s_long / 10, v_long)
        ...
        return self.M * (div - (u*(1-u)*(2*u-1)) + energy)
        
    def forward(self, x):
        output, s_long, v_long = self.model(x)
        # v (B, 2, H, W)を正規化
        ...
        for _ in range(self.steps):
            ...
            k1 = self.allen_cahn(x, s_long, v_long, x_copy * 0.01)
            k2 = self.allen_cahn(x + self.dt/2 * k1, s_long, v_long, x_copy * 0.01)
            k3 = self.allen_cahn(x + self.dt/2 * k2, s_long, v_long, x_copy * 0.01)
            k4 = self.allen_cahn(x + self.dt * k3, s_long, v_long, x_copy * 0.01)
            x = x + self.dt/6 * (k1 + 2*k2 + 2*k3 + k4)
            
        # 相分離モデル化のために[0, 1]にクリッピング
        x = torch.clamp(x, 0, 1)
        return x, s_long, s_long / 10, v_long