...
def crop2fullmask(crop_mask, bbox, im=None, im_size=None, zero_pad=False, relax=0, mask_relax=True,
                  interpolation=cv2.INTER_CUBIC, scikit=False, bg_value=20):
    if scikit:
    ...
    crop_mask_resized = cv2.resize(crop_mask, (im_si[1], im_si[0]), interpolation=interpolation)

    result = np.ones(im_si) * bg_value

    result[:crop_mask_resized.shape[0], :crop_mask_resized.shape[1]] = crop_mask_resized
...
def fixed_resize(sample, resolution, flagval=None):
    ...
    if isinstance(resolution, int):
        ...
        # 縦横比を維持したまま解像度変換(低い方の解像度はresolutionとする)
...
def crop_from_mask(img, mask, relax=0, zero_pad=False):
    ...
    # mask > 0のx_min, y_min, x_max y_maxを取得
    bbox = get_bbox(mask, pad=relax, zero_pad=zero_pad)

    if bbox is None:
        return None
    # imgをmaskのbboxの大きさに切り取ってcropとする
