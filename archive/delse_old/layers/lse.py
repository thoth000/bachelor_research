...
def levelset_evolution(phi, vf, g=None, T=5, timestep=5, dirac_ratio=0.3, dt_max=30, _test=False, _normalize=False):
    if _normalize:
        vf = F.normalize(vf, p=2, dim=1) # L2ノルムで正規化
    # ベクトル場vfのx方向, y方向成分への分割
    vy = vf[:, 0, :, :].unsqueeze(1)
    vx = vf[:, 1, :, :].unsqueeze(1)

    for k in range(T):
        phi = NeumannBoundCond(phi) # レベルセット関数にNeumann境界条件を適用
        [phi_y, phi_x] = gradient_sobel(phi, split=True) # レベルセット関数の勾配(x, y方向)を計算
        s = torch.sqrt(torch.pow(phi_x, 2) + torch.pow(phi_y, 2) + 1e-10) # 正規化係数
        Nx = phi_x / s
        Ny = phi_y / s
        curvature = div(Nx, Ny) # 曲率curvatureを計算
        diracPhi = Dirac(phi, dirac_ratio) # Dirac関数を計算
        motion_term = vx * phi_x + vy * phi_y # Motion Term(レベルセット更新項の一つ: 外部項)を計算

        # レベルセット関数の更新: Motion Termと曲率に基づいて更新
        if g is None: # gはレベルセットを制御するための追加のスカラー場
            phi = phi + timestep * diracPhi * (motion_term + curvature.detach())
            phi = phi + 0.2 * distReg_p2(phi.detach()) # 更新後に距離正規化項を適用
        else: # 重み付け以外は上と同様
