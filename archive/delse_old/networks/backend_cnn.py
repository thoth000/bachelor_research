...
def backend_cnn_model(args):
    ...
    if args.backend_cnn == 'resnet101':
        model = build_resnet_model(n_classes=(1, 2), pretrained=False, nInputChannels=args.input_channels,
                                   classifier=args.classifier)
    
    elif args.backend_cnn == 'resnet50':
        model = build_resnet_model(n_classes=(1, 2), pretrained=False, nInputChannels=args.input_channels,
                                   classifier=args.classifier, layers=(3, 4, 6, 3))
    
    elif args.backend_cnn == 'resnet101-skip':
        concat_dim = args.concat_dim
        resnet = build_resnet_model(n_classes=(1, 2), pretrained=False, nInputChannels=args.input_channels,
                                    classifier=args.classifier, feature_dim=4*concat_dim)
        model = SkipResnet(concat_channels=concat_dim, resnet=resnet)
        
    elif args.backend_cnn == 'resnet101-skip3':
        concat_dim = args.concat_dim    # 128
        resnet = build_resnet_model(n_classes=(1, 2, 1), pretrained=False, nInputChannels=args.input_channels,
                                    classifier=args.classifier, feature_dim=4*concat_dim)
        model = SkipResnet(concat_channels=concat_dim, resnet=resnet)
    
    elif args.backend_cnn == 'resnet101-skip-pretrain':
        concat_dim = args.concat_dim    # 128
        resnet = build_resnet_model(n_classes=(1, 2, 1), pretrained=True, nInputChannels=args.input_channels,
                                    classifier=args.classifier, feature_dim=4*concat_dim)
        model = SkipResnet(concat_channels=concat_dim, resnet=resnet)
    
    elif args.backend_cnn == 'resnet101-pretrain':
        model = build_resnet_model(n_classes=(1, 2, 1), pretrained=True, nInputChannels=args.input_channels,
                                   classifier=args.classifier)
