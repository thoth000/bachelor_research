...
class Anisotropic_Diffusion(nn.Module):
    ...
    def allen_cahn(self, u, s_long, v_long):
        # Allen-Cahn方程式の計算
        # u (torch.Tensor): (B, C, H, W)
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            ...
            k1 = self.allen_cahn(x, s_long, v_long)
            k2 = self.allen_cahn(x + self.dt/2 * k1, s_long, v_long)
            k3 = self.allen_cahn(x + self.dt/2 * k2, s_long, v_long)
            k4 = self.allen_cahn(x + self.dt * k3, s_long, v_long)
