...
class FR_UNet(nn.Module):
    def __init__(self,  args):
        ...
        self.final1 = nn.Conv2d(
            filters[0], self.num_classes * 4, kernel_size=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        # [batch, num_classes * 4, H, W] -> [batch, num_classes, 4, H, W] に変形
        output = output.view(output.size(0), self.num_classes, 4, output.size(2), output.size(3))

        # 各スカラー場に分割
        phi_0, m, vx, vy = output[:, :, 0], output[:, :, 1], output[:, :, 2], output[:, :, 3]
        m = torch.sigmoid(m)
        
        return phi_0, m, vx, vy
