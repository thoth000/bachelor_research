...
def check_args():
    ...
    # モデル固有のパラメータ
    parser.add_argument('--num_classes', type=int, default=1) # 分類クラス数
    parser.add_argument('--num_channels', type=int, default=1) # 入力画像のチャンネル数
    ...
    parser.add_argument('--fix_pretrained_params', type=bool, default=False)
    parser.add_argument('--pre_pass', type=bool, default=False)
...
def main():
    ...
    train(args, writer)
    # argsにpretrained_pathを追加
    args.pretrained_path = os.path.join(args.save_dir, "final_model.pth")
    
    acc_0, sen_0, spe_0, iou_0, miou_0, dice_0, acc, sen, spe, iou, miou, dice = test(args)
    if args.rank == 0:
        metrics = {'acc_0': acc_0, 'sen_0': sen_0, 'spe_0': spe_0, 'iou_0': iou_0, 'miou_0': miou_0, 'dice_0': dice_0,
                   'acc': acc, 'sen': sen, 'spe': spe, 'iou': iou, 'miou': miou, 'dice': dice}
