    ...
    with torch.no_grad():
        for i, sample in enumerate(dataloader):
            ...
            if epoch < args.pre_epoch or args.pre_pass:
                ...
                phi_T = levelset_evolution(phi_0, vx, vy, m)
                # vy_gt, vx_gt = gradient_sobel(sample['dt'].to(device), split=True)
                loss = LSE_output_loss(phi_T, sample['transformed_mask'].to(device))
            ...
            if args.rank == 0 and (args.save_mask or epoch == args.max_epoch - 1) and i < 2:
                ...
                # save_main_out_image(m, os.path.join(out_dir, f"{i}_m.png"))
                # save_vector_field(sample['sdt'], vx, vy, os.path.join(out_dir, f"{i}_vf.png"))
                # save_vector_field(sample['sdt'], vx_gt, vy_gt, os.path.join(out_dir, f"{i}_vf_gt.png"))
