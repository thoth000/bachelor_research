...
class FR_UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        ex = (self.ex1(x1_1)+self.ex2(x10)+self.ex3(x11)+self.ex4(x12)+self.ex5(x13)) / 5
...
class Cahn_Hilliard_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            ...
            k1 = self.cahn_hilliard_ex(x, ex)
            k2 = self.cahn_hilliard_ex(x + self.dt/2 * k1, ex)
            k3 = self.cahn_hilliard_ex(x + self.dt/2 * k2, ex)
            k4 = self.cahn_hilliard_ex(x + self.dt * k3, ex)
